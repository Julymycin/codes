...
class LogGabor(Image):
    ...
    def init(self):
        ...
        self.sf_0 = .5 * (1 - 1 / self.n_levels) / np.logspace(0, self.n_levels - 1, self.n_levels,
                                                               base=self.pe.base_levels, endpoint=False)
        ...
        for i_sf_0 in range(len(self.sf_0)):
            ...
            if mask:
                ...
                circ = Ellipse((.5 * self.pe.N_Y, .5 * self.pe.N_X),
                               ...
                               fill=False, facecolor='none', edgecolor='black', alpha=0.5, ls='dashed',
                               lw=linewidth_mask)
                ax.add_patch(circ)
            if spiral:
